...
_ ** If the search command is not executed on the server, i.e. it is executed on the client, then the server host must be specified with the_ -H ldap://dirIP. For example: _ldapsearch -x -H ldap://192.168.3.208 -b dc=example,dc=com -D "cn=admin,dc=example,dc=com" -W_
...
https://askubuntu.com/questions/127389/how-to-configure-ubuntu-as-an-ldap-client <br>
